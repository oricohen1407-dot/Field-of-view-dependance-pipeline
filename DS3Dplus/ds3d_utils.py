...
def circular_aperture_from_diameter(
    grid_size,
    pixel_size_um,
    diameter_mm,
):
    """
    Create a centered circular aperture using a physical diameter.
    ...
    pixel_size_um : float
        Physical sampling interval in the mask plane [um/pixel].
    diameter_mm : float
        Physical aperture diameter [mm].

    Returns
    -------
    np.ndarray
        Float32 aperture with shape (grid_size, grid_size).
    """
    grid_size = int(grid_size)
    pixel_size_um = float(pixel_size_um)
    diameter_um = float(diameter_mm) * 1000.0
    ...
    coordinates_um = (np.arange(grid_size, dtype=np.float32) - (grid_size - 1) / 2.0 ) * pixel_size_um
    xx_um, yy_um = np.meshgrid( coordinates_um, coordinates_um, indexing="xy",)

    radius_um = diameter_um / 2.0
    aperture = (xx_um**2 + yy_um**2 <= radius_um**2)
...
class ImModel(nn.Module):
    def __init__(self, params):
        ...
        self.mask_iris_diameter_mm = float(params["mask_iris_diameter_mm"] )
        mask_iris = circular_aperture_from_diameter( grid_size=N, pixel_size_um=ps_BFP, diameter_mm=self.mask_iris_diameter_mm,)
        self.mask_iris = torch.from_numpy(mask_iris ).to(device)
    ...
    def get_psfs(self, xyzps):  # each batch can only have the same number of particles
        ...
        ef_off = ef_off * phase * iris_mask
        ef_off = torch.where(iris_mask, ef_off, 0)
...
class ImModelBase(nn.Module):
    def __init__(self, params):
        ...
        N = int(N + 1 - (N % 2))  # make it odd0
        self.mask_iris_diameter_mm = float(params["mask_iris_diameter_mm"] )
        mask_iris = circular_aperture_from_diameter(grid_size=N, pixel_size_um=ps_BFP, diameter_mm=float(params["mask_iris_diameter_mm"]),)
        self.mask_iris = torch.from_numpy(mask_iris).to(device=self.device, dtype=torch.float32,)
        # pupil/aperture at back focal plane
        d_pupil = 2 * f_4f * NA / np.sqrt(M ** 2 - NA ** 2)  # diameter [um]
        pn_pupil = d_pupil / ps_BFP  # pixel number of the pupil diameter should be smaller than the simulation size N
        if N < pn_pupil:
    ...
    def get_psfs(self, xyzps):  # each batch can only have the same number of particles
        ...
        phase_lateral_sub_pixel = self.Xgrid * x_sub.unsqueeze(1) + self.Ygrid * y_sub.unsqueeze(1)
        phase_lateral_coarse = self.Xgrid * (x_coarse + cx * self.ps_camera / self.M).unsqueeze(1) + self.Ygrid * (
                    y_coarse + cy * self.ps_camera / self.M).unsqueeze(1)
        ...
        for ii in range(len(dx_mask_px)):
            ef_mask_shifted.append(
                shift_complex_field_integer(ef_mask[ii, :, :], int(dx_mask_px[ii].item()), int(dy_mask_px[ii].item())))
        ...
        for ii in range(len(dx_mask_px)):
            ef_mask_unshifted.append(shift_complex_field_integer(ef_mask_shifted[ii, :, :], int(-dx_mask_px[ii].item()),
                                                                 int(-dy_mask_px[ii].item())))
        ...
        ef_bfp_after = ef_bfp_after * torch.exp(1j * (phase_lateral_coarse)).to(
            torch.complex64)  # adding coarse lateral phase
...
class ImModelTraining(ImModelBase):
    ...
    def psf_patch_clean(self, xyzp_np):
        ...
        ef_mask_shifted = ef_mask_shifted * phase * iris_mask
        ef_mask_shifted = torch.where(iris_mask > 0.5, ef_mask_shifted, 0)
        ...
        ef_mask_unshifted.append(
            shift_complex_field_integer(ef_mask_shifted[0], int(-dx_mask_px.item()), int(-dy_mask_px.item())))
    ...
    def __init__(self, param_dict):
        ...
        self.mask_offset_in_um = float(
            param_dict.get('mask_offset_in_um', 0.0))  # added on 2/01/2026 for mask displacement optimization
        self.centralBeadCoordinates_pixel = param_dict.get('centralBeadCoordinates_pixel', [self.H / 2,
                                                                                            self.W / 2])  # added on 2/01/2026 for mask displacement optimization
    ...
    def forward(self, xyzps):
        ...
        if self.non_uniform_noise_flag:
            ...
            try:
                ...
                im = im + torch.round(
                    read_baseline + torch.randn(im.shape, device=self.device) * read_std)  # ori's edit
...
class Sampling():
    ...
    def show_volume(self, ):
        ...
        for i in range(xyz_ids.shape[0]):
            ...
            y[zidx:zidx + 2 * self.blob_r + 1, yidx:yidx + 2 * self.blob_r + 1, xidx:xidx + 2 * self.blob_r + 1] += \
            blob3d[i]
...
class Volume2XYZ(nn.Module):
    def __init__(self, params):
        ...
        self.maxpool = MaxPool3d(kernel_size=2 * self.r + 1, stride=1, padding=self.r)  # for maxpooling (22/04/2026)
    ...
    def local_avg(self, xbool, ybool, zbool, pred_vol_pad):
        ...
        pred_vol_all_ = pred_vol_pad[0][all_z, all_y, all_x].view(num_pts, self.r * 2 + 1, self.r * 2 + 1,
                                                                  self.r * 2 + 1)
    ...
    def forward(self, pred_vol):
        ...
        conf_vol = self.maxpool(pred_thresh)  # for maxpooling: removed on 20/04/2026. maxpool is suspected to cause the comb
        #conf_vol = pred_thresh  # added on 22/04/2026 for weightening instead of maxpooling
...
class LON(nn.Module):
    def __init__(self, D, us_factor, maxv):
        ...
        C = 64*2  #originally 64! changed on 01/06/2026
        ...
        self.norm_img = nn.BatchNorm2d(num_features=1,
                                       affine=True)  # added on 06/02/2026 to improve fov dependance training
